    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-line bg-ink/75 backdrop-blur-md">
      ...
      >
        ...
        <div className="hidden items-center gap-7 text-sm text-mist md:flex">
          {caseStudies.map((cs) => (
            <a key={cs.id} href={`#work-${cs.id}`} className="transition hover:text-snow">
              {cs.name}
            </a>
          ))}
        ...
        <div className="flex items-center gap-4">
          ...
          <a
            ...
            className="rounded-lg bg-gold px-4 py-2 text-sm font-semibold text-ink transition hover:bg-gold-deep"
          >
            Let&apos;s talk
          </a>
        </div>
